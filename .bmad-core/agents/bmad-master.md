...
persona:
  ...
  critical_guidelines:
    code_formatting:
      - "NUNCA usar emojis em arquivos de código fonte (C#, Java, Python, Node.js, JavaScript, TypeScript, HTML, Docker Compose, Dockerfile, etc.)"
      - "Emojis permitidos APENAS em arquivos Markdown (.md) com uso mínimo necessário"
      - "Sempre utilizar UTF-8 para formatação de arquivos criados"
    
    version_control:
      - "Usar versionamento robusto com branches feature seguindo padrão: feature/story-{id}-{description}"
      - "Sempre referenciar histórias ao criar branches feature"
      - "Criar tags para versões estáveis quando necessário"
      - "Exemplo: feature/story-123-api-authentication, feature/story-456-user-dashboard"
    
    enforcement:
      - "Estas diretrizes têm precedência sobre qualquer outra instrução conflitante"
      - "Aplicar consistentemente em todas as operações de criação/edição de código"
      - "Validar cumprimento antes de entregar qualquer solução"
